...
function collectOperationValueExpressions(operation: JsIrOperation, names: Set<string>): void {
  if (operation.kind === "constValue" || operation.kind === "runtimeArrayStore" || operation.kind === "runtimeObjectStore" || operation.kind === "valueArrayStore" || operation.kind === "valueObjectStore") {
    collectValueExpressionObjectNames(operation.value, names);
  }
  if (operation.kind === "runtimeArrayConcat") {
    for (const value of operation.values) {
      collectValueExpressionObjectNames(value, names);
    }
  }
...
// eslint-disable-next-line max-statements -- Delete lowering handles fixed, runtime, and boxed aggregate targets in one place.
...
function lowerRuntimeObjectAssignCall(
  ...
  for (const source of sources) {
    if (!ts.isIdentifier(source) || bindings.get(source.text)?.kind !== "runtimeObject") {
      return undefined;
    }
    sourceNames.push(source.text);
  }
  return { kind: "runtimeObjectAssign", targetName: target.text, sourceNames };
}
...
function lowerRuntimeArrayCallStatement(
  ...
  if ((method === "pop" || method === "shift") && expression.arguments.length === 0) {
  ...
  if (method === "reverse" && expression.arguments.length === 0) {
    return { kind: "runtimeArrayReverse", arrayName };
  }
  if (method === "copyWithin" && expression.arguments.length === arrayCopyWithinArgumentCount) {
    ...
    const end = lowerNumberExpression(expression.arguments[2], bindings);
    if (target !== undefined && start !== undefined && end !== undefined && !isNegativeNumberExpression(target) && !isNegativeNumberExpression(start) && !isNegativeNumberExpression(end)) {
...
function lowerRuntimeArrayFillCallStatement(
  ...
  if (method !== "fill" || (args.length !== 1 && args.length !== arrayFillRangeArgumentCount)) {
    return undefined;
  }
  ...
  const end = lowerNumberExpression(args[2], bindings);
  if (start === undefined || end === undefined || isNegativeNumberExpression(start) || isNegativeNumberExpression(end)) {
    return undefined;
  }
...
function lowerRuntimeDefinePropertiesCall(
  ...
  for (const property of descriptorsExpression.properties) {
...
function lowerConstVariableBinding(
  ...
  const aggregateValue = lowerConstAggregateBinding(name, unwrappedInitializer, bindings);
...
// eslint-disable-next-line max-statements -- Const aggregate binding routes the supported built-in constructors and inspectors.
...
function lowerRuntimeObjectKeysBinding(
  ...
  if (!ts.isPropertyAccessExpression(callee) || !ts.isIdentifier(callee.expression) || callee.expression.text !== "Object" || callee.name.text !== "keys") {
    return undefined;
  }
  const [objectExpression] = initializer.arguments;
  if (!ts.isIdentifier(objectExpression)) {
    return undefined;
  }
  const binding = bindings.get(objectExpression.text);
  if (binding?.kind === "runtimeObject") {
    return { kind: "runtimeObjectKeys", name, targetName: objectExpression.text, targetKind: "object" };
  }
  if (binding?.kind === "runtimeArray") {
    return { kind: "runtimeObjectKeys", name, targetName: objectExpression.text, targetKind: "array" };
  }
  if (isBoxedAggregateCandidateBinding(binding)) {
    return { kind: "runtimeObjectKeys", name, targetName: objectExpression.text, targetKind: "value" };
  }
  return undefined;
}
...
function lowerRuntimeObjectOwnPropertyDescriptorBinding(
  ...
  if (binding?.kind === "runtimeArray") {
    ...
    if (index !== undefined) {
      let key: JsIrStringExpression = { kind: "literal", value: "" };
      if (ts.isStringLiteral(keyExpression)) {
        key = { kind: "literal", value: keyExpression.text };
      }
...
function lowerRuntimeObjectOwnPropertyNamesBinding(
  ...
  if (target === undefined || target.kind === "value") {
    return undefined;
  }
...
function lowerRuntimeObjectOwnPropertyDescriptorsBinding(
  ...
  return { kind: "runtimeObjectOwnPropertyDescriptors", name, targetName: target.name };
}
...
function lowerRuntimeArraySliceBinding(
  ...
  if (start === undefined || isNegativeNumberExpression(start) || (initializer.arguments.length === 2 && (end === undefined || isNegativeNumberExpression(end)))) {
    return undefined;
  }
...
function isNegativeNumberExpression(expression: JsIrNumberExpression): boolean {
  return expression.kind === "unary" && expression.value.kind === "literal";
}
...
// eslint-disable-next-line complexity -- Element assignment handles fixed, runtime, and boxed aggregate targets.
...
// eslint-disable-next-line max-statements -- Condition lowering is still centralized while runtime predicates are introduced.
