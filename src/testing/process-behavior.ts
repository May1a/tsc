...
// The Node wrappers report uncaught throws as structured JSON on a dedicated
// stderr line so error class and message can be compared against the native
// executable's uncaught-error output.
export const thrownSentinel = "__TSCN_NODE_THROWN_V1__";

const parseThrownObservation = (json: string): ThrownObservation => {
  let parsed: unknown;
  try {
    parsed = JSON.parse(json);
  } catch {
    throw new Error(`${thrownSentinel} record is not valid JSON: ${JSON.stringify(json)}`);
  }
  ...
  throw new Error(`${thrownSentinel} record does not match the ThrownObservation payload: ${JSON.stringify(json)}`);
};
...
export const nodeModuleWrapperSource = `
